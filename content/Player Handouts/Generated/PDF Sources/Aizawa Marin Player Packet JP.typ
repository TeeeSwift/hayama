// Generated from content/Player Handouts/Character Packets JP/Aizawa Marin.md
// Edit the Markdown source, then rerun scripts/build-player-handouts-typst.mjs.

#set document(title: "Aizawa Marin Player Packet JP")
#set page(
  paper: "a4",
  binding: left,
  margin: (inside: 22mm, outside: 14mm, top: 16mm, bottom: 18mm),
  numbering: "1",
)
#set text(
  font: ("Noto Sans CJK JP", "Hiragino Sans", "Yu Gothic", "Arial"),
  size: 10.5pt,
  lang: "ja",
)
#set par(justify: false, leading: 0.62em)
#set heading(numbering: none)
#show heading.where(level: 1): it => block(above: 0pt, below: 10pt, text(size: 20pt, weight: "bold", it.body))
#show heading.where(level: 2): it => block(above: 14pt, below: 6pt, text(size: 14pt, weight: "bold", it.body))
#show heading.where(level: 3): it => block(above: 10pt, below: 4pt, text(size: 11.5pt, weight: "bold", it.body))
#show table: set text(size: 9pt)

= Aizawa Marin

この資料は、セッション中に「何ができるか」を思い出すための早見表です。数字で迷ったらキャラクターシートを優先してください。

== まず覚えること

- *クラス:* ドルイド
- *背景:* 侍祭
- *最大HP:* 9
- *呪文能力値:* 判断力
- *呪文セーヴ難易度:* 13
- *呪文攻撃ボーナス:* +5
- *呪文スロット:* 1レベル呪文スロット2つ
- *キャントリップ:* Druidcraft、Shillelagh
- *常に意識する呪文:* Speak with Animals
- *得意なセーヴ:* 知力、判断力
- *受動知覚:* 13
- *受動看破:* 15

== ターンの基本

自分のターンでは、だいたい次のものを1回ずつ使えます。

- *アクション:* 攻撃、呪文、多くの技能行動
- *ボーナスアクション:* 一部の呪文や特徴が指定する行動
- *移動:* 移動速度ぶん動く
- *リアクション:* 条件が起きた時に使う反応

== ドルイドとしての動き方

あなたは「自然、身体、天候、動物、場所の変化を読む」タイプです。

- 戦闘では、まず敵の動きや地形を変えられるか考える。
- 味方が危ない時は回復を使う。
- Shillelaghで武器を判断力ベースにして戦える。
- 動物や自然現象から情報を集める。
- 準備呪文は長休み後に入れ替えられるので、次の日の予定に合わせる。

== クラス特徴

=== Druidic

ドルイドの秘密の言語や自然との結びつきを持っています。2024年ルールでは、Speak with Animalsが常に準備される扱いです。

=== 呪文準備

ドルイドは、長休み後に1レベル以上の準備呪文を変えられます。まだ固定しなくて大丈夫です。

初心者向けの準備例:

- *Healing Word:* 遠くの味方をボーナスアクションで回復
- *Entangle:* 敵を止める
- *Detect Magic:* 魔法の気配を探す
- *Goodberry:* 探索後の回復と食料

== 選んだ呪文

#table(
  columns: (1fr, 1fr, 1fr, 1fr),
  inset: 5pt,
  stroke: 0.45pt + rgb("#D0D5DD"),
  fill: (x, y) => if y == 0 { rgb("#F2F4F7") } else { white },
  strong([呪文]),
  strong([種類]),
  strong([使う場面]),
  strong([注意]),
  [Druidcraft],
  [キャントリップ],
  [天候、植物、小さな自然現象を扱う時],
  [ダメージはない。雰囲気と情報に強い],
  [Shillelagh],
  [キャントリップ],
  [杖や棍棒で戦う時],
  [判断力で攻撃しやすくなる],
  [Speak with Animals],
  [常に意識],
  [動物が目撃者、案内役、警告役になりそうな時],
  [動物は動物の視点で答える],
)

== 戦闘で迷ったら

+ 敵を止める方がよいか、味方を助ける方がよいかを考える。
+ 前に出るならShillelagh。
+ 味方が倒れそうならHealing WordやCure Woundsを考える。
+ 場所や自然の異変に関係しそうなら、ドルイドらしい観察をDMに聞く。

== 背景メモ

Marinは地元の高校生で、週末には父のダイビング指導を手伝っています。海に対して、軽い憧れではなく、規律、慎重さ、敬意を持っています。

彼女にとって海は遊び場である前に、向き合うべき相手です。

== 最近の出来事

- ダイビング中、海の感覚が突然変わった。
- 水圧、音、浮力、身体の感覚が普通ではなかった。
- 海に引きずり込まれたというより、「海の方が自分を受け入れすぎた」ように感じた。
- 父に引き戻されたあと、地上の方が不自然に重く感じられた。

== 関係

- *父:* ダイビング指導者。Marinにとって海への姿勢を教えた大事な存在。
- *逗子海岸で教えている友人:* もっと軽く、自由に、見栄えよく海と関わっている。その自由さに少し嫉妬がある。
- *海:* 敬意を払う対象。最近は、ただの自然以上に自分へ反応している気がする。

== セッション中に聞いていいこと

- 「この自然現象は普通ですか？」
- 「動物や海の生き物に話しかけられますか？」
- 「この呪文は集中が必要ですか？」
- 「長休み後にこの準備呪文へ変えられますか？」
